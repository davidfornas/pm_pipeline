...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
	  ...
	  //DF 2 for convenience beacuse this callback is much slower now because of resolution
	  if(counter < 2){
		counter++;
	  }else{
	  	//Track the object
...
main (int argc, char** argv)
{
  ...
  //DF
  ros::init(argc, argv, "tutorial_processing");
  ros::NodeHandle nh;
  ...
  pcl::visualization::CloudViewer* viewer_ = new pcl::visualization::CloudViewer("Tracking Viewer");
  ...
  // TOPIC GRABBER
  //ros::Subscriber sub = nh.subscribe ("/stereo_down/points2", 1, cloud_cb);
  ...
  //OPENNI interface->start();
  while (!viewer_->wasStopped ()){
	  ros::spinOnce();
    boost::this_thread::sleep(boost::posix_time::seconds(1));}
  //OPENNI interface->stop();
}
