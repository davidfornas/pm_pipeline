...
int main(int argc, char** argv)
{
  ...
  if (!pcd_filename_indices.empty())
  {
    ...
    scene_sensor_pose = Eigen::Affine3f(
        Eigen::Translation3f(point_cloud.sensor_origin_[0], point_cloud.sensor_origin_[1],
                             point_cloud.sensor_origin_[2])) * Eigen::Affine3f(point_cloud.sensor_orientation_);

    // Far ranges usage is optional
    //std::string far_ranges_filename = pcl::getFilenameWithoutExtension (filename)+"_far_ranges.pcd";
    //if (pcl::io::loadPCDFile(far_ranges_filename.c_str(), far_ranges) == -1)
    //  std::cout << "Far ranges file \""<<far_ranges_filename<<"\" does not exists.\n";

    clock_t end = clock();
    ROS_DEBUG_STREAM("Loading elapsed seg. time: " << double(end - begin) / CLOCKS_PER_SEC);
  }
  ...
  clock_t end = clock();
  ROS_DEBUG_STREAM("Computing elapsed seg. time: " << double(end - begin) / CLOCKS_PER_SEC);
  ...
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointWithRange> veil_points_color_handler(veil_points_ptr, 255,
                                                                                                  0, 0);
