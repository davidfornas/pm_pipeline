...
public:
  ...
  void applyFurthest( CloudPtr & mirrored ){
    estimateFurthest();
    apply(mirrored);
  }

  void applyCentroid( CloudPtr & mirrored ){
    estimateCentroid();
    apply(mirrored);
  }
...
public:
  ...
  double estimateAndApply( CloudPtr & mirrored ){
    estimateAxis();
    return apply(mirrored);
  }
