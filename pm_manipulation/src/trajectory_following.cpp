...
void TrajectoryFollowing::moveToNextWaypoint(){
  ...
  std::cout << "Desired end effector position" << std::endl << target << std::endl;
  ...
  std::cout << "Resulting configuration" << std::endl << joints << std::endl;

  //Publish FK of found joint config
  tf::StampedTransform fk(VispTools::tfTransFromVispHomog(target_fk), ros::Time::now(), "/kinematic_base", "/reachable_cMg");//kinematic_base <=> vehicle
