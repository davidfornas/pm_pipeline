...
int main(int argc, char** argv)
{
  ...
  if( argc != 3 ){
    ROS_ERROR_STREAM("rosrun pm_apps colour_cloud /sense3d/scan /sense3d/scan/color");
  }
  std::string input_cloud_topic(argv[1]), output_cloud_topic(argv[2]);

  cloud_pub = nh.advertise<sensor_msgs::PointCloud2>( output_cloud_topic, 1000);
  ros::Subscriber original_sub	= nh.subscribe<sensor_msgs::PointCloud2>( input_cloud_topic, 1, cloudCallback);
